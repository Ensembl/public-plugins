...
package EnsEMBL::Web::RunnableDB::Forger;

### Hive Process RunnableDB for Forger
...
use EnsEMBL::Web::Parsers::Forger;
...
sub fetch_input {
  ...
  throw exception('HiveException', 'Forger script file is either missing or not accessible.') unless -r "$code_root/$script_path";
  throw exception('HiveException', 'Input file for Forger could not be located.') unless -r "$work_dir/$input_file";
...
sub write_output {
  ...
  $self->save_results($job_id, {}, EnsEMBL::Web::Parsers::Forger->new($self)->parse($output_file));
