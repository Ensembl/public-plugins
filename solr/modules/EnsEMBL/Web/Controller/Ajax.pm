...
sub ajax_psychic { # Invoke psychic via AJAX, to see if we need to redirect.
  ...
  # NOTE: as of April 15 2021, psychic responds differently to different order of query parameters in the url
  # It does not account for the species parameter if it follows the q parameter.
  my $full_query = join '&', grep { !!$_ } ($species_parameter, $query_parameter);
