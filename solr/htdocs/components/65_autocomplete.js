...
(function() {
  ...
  sp_names = function(name, callback) {
    ...
    return ajax_json("/Multi/Ajax/species", {name}, function(data) {
      var ref;
      if (data.favs != null) {
        sp_fav = data.favs;
      }
  ...
  score_of = function(doc, favs) {
    ...
    score += (((ref = doc.location) != null ? ref.indexOf('_') : void 0) !== -1 ? 0 : 10); // XXX is_reference
  ...
  $.widget('custom.searchac', $.ui.autocomplete, {
    _create: function() {
      ...
      this.element.on('change keypress paste focus textInput input', (e) => {
        var val;
      ...
      return this.element.on('keydown', (e) => {
        var gval, val;
        if (e.keyCode === 39) { // right arrow
          val = this.element.val();
          gval = ghost.val();
          if (gval && gval.substring(0, val.length) === val && this.element.getCursorPosition() === val.length) {
            return this.element.val(gval);
          }
        }
      });
    },
