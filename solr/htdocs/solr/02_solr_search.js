...
(function() {
  var Hub, Renderer, Request, SearchTableState, all_requests, body_main_requests, body_requests, body_species_homepage_request, code_select, current_update_seq, dispatch_all_requests, dispatch_facet_request, dispatch_main_requests, double_trap, each_block, generate_block_list, rate_limiter, remote_log, xhr_idx, _clone_array, _clone_object, _kv_copy,
    __slice = [].slice,
  ...
  body_main_requests = function() {
    var blocks, extras, favs, rigid, x, _i, _len,
      _this = this;
    rigid = [];
      ...
      blocks.push((function(xx) {
        return function(request, q, start, len) {
          if (start === -1) {
            ...
            return request.request_results(q, xx, start, len);
          }
        };
      })(x));
    }
  ...
  body_requests = [body_species_homepage_request, body_main_requests];

  dispatch_main_requests = function(request, state, table, query, update_seq) {
    var b, blocks, ret, t, total, _i, _len,
      _this = this;
    t = table.xxx_table();
    t.reset();
    blocks = [];
    ...
    total = 0;
    ret = each_block(blocks.length, function(i) {
    ...
    return ret.then(function(sizes) {
      var i, local_offset, offset, requests, results, rows_left, tops, _j, _ref;
      ...
      results = each_block(blocks.length, function(i) {
        if (requests[i][0] !== -1) {
          return blocks[i](request, query, requests[i][0], requests[i][2]);
        }
      });
      tops = [];
      results = results.then(function(docs_frags) {
        var _k, _ref1;
        if (state.start() === 0) {
          for (i = _k = 0, _ref1 = requests.length; 0 <= _ref1 ? _k < _ref1 : _k > _ref1; i = 0 <= _ref1 ? ++_k : --_k) {
            if (requests[i][0] !== -1 && tops.length < state.pagesize()) {
  ...
  Request = (function() {
    ...
    Request.prototype.request_results = function(params, extra, start, rows, every) {
      ...
      return this.raw_ajax(input).then(function(data) {
        var docs, num, table, _ref1, _ref2, _ref3, _ref4;
