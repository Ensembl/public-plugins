...
(function() {
  var Hub, Renderer, Request, SearchTableState, all_requests, code_select, current_update_seq, dispatch_all_requests, dispatch_facet_request, dispatch_main_requests, double_trap, each_block, generate_block_list, rate_limiter, remote_log, xhr_idx, _clone_array, _clone_object, _kv_copy,
    __slice = [].slice,
  ...
  dispatch_main_requests = function(request, state, table, cols, query, start, rows, update_seq) {
  ...
  dispatch_facet_request = function(request, state, table, cols, query, start, rows, update_seq) {
    var fq, k, params,
      _this = this;
  ...
  dispatch_all_requests = function(request, state, table, start, rows, cols, filter, order, update_seq) {
    ...
    $.each(all_requests, function(k, v) {
      plugin_list.push(k);
      return plugin_actions.push(v(request, state, table, cols, input, start, rows, update_seq));
    });
  ...
  Request = (function() {
    ...
    Request.prototype.render_table = function(table, state) {
      ...
      return dispatch_all_requests(this, state, table, start, page, state.columns(), state.filter(), state.order(), current_update_seq);
    };
