...
sub _messages { ## TODO change message texts ## TODO provide error or message heading along with the text
  ...
  MESSAGE_OPENID_CANCELLED      => 104 => sprintf('Your request to login via %s was cancelled. Please try again, or use one of the alternative login options below.', $hub->param('provider') || 'OpenID'),
  MESSAGE_OPENID_INVALID        => 128 => '_message__OPENID_INVALID',
  MESSAGE_OPENID_SETUP_NEEDED   => 167 => '_message__OPENID_SETUP_NEEDED',
  MESSAGE_OPENID_ERROR          => 178 => '_message__OPENID_ERROR',
  ...
  MESSAGE_CONFIRMATION_FAILED   => 314 => 'The email address could not be confirmed.',
  ...
  MESSAGE_EMAILS_INVALID        => 510 => sprintf('Invalid email address: %s', $hub->param('invalids') || ''),
  ...
  MESSAGE_VERIFICATION_SENT     => 528 => sprintf(q(A verification email has been sent to the email address '%s'. Please go to your inbox and click on the link provided in the email.), $hub->param('email')),
  MESSAGE_PASSWORD_EMAIL_SENT   => 602 => sprintf(q(An email has been sent to the email address '%s'. Please go to your inbox and follow the instructions to reset your password provided in the email.), $hub->param('email')),
  MESSAGE_EMAIL_CHANGED         => 645 => sprintf(q(You email address on our records has been successfully changed. Please <a href="%s">%s</a> to continue.), $hub->user ? 'click here' : 'login', $hub->url({'action' => 'Preferences'})),
