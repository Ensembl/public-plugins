...
sub content {
  ...
  my @invitations = map({ $_->is_pending_invitation
    ? sprintf(q{<li><i>%s (%s)</i> is inviting you to join the group <i>%s</i><br />%s &middot; %s &middot; %s</li>},
        ...
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'Accept',     'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Accept'}),
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'Decline',    'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Decline'}),
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'BlockGroup', 'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Block group from sending further invitations'})
      )
    : ()
    } @{$user->memberships}
  );
  ...
  my @requests = map({ $_->is_pending_request
    ? sprintf(q{<li><i>%s (%s)</i> would like to join your group <i>%s</i><br />%s &middot; %s &middot; %s</li>},
        ...
        $self->js_link({'href' => {'action' => 'Groups',     'function' => 'View',       'id' => $_->group->group_id}, 'target' => 'page', 'caption' => $self->html_encode($_->group->name)}),
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'Allow',      'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Allow'}),
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'Ignore',     'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Ignore'}),
        $self->js_link({'href' => {'action' => 'Membership', 'function' => 'BlockUser',  'id' => $_->group_member_id}, 'target' => 'page', 'caption' => 'Block user from sending further requests'})
      )
    : ()
