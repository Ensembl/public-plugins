...
sub process {
  ...
  my $hub               = $self->hub;
  my $login             = $object->fetch_login_from_url_code(1) or return $self->redirect_message('MESSAGE_LOGIN_MISSING', {'error' => 1});
...
sub redirect {
  ...
  return $self->hub->redirect({'action' => 'OpenID', 'function' => $function, $error ? ('err' => $self->object->get_message_code($error)) : (), %$url_params});
}
