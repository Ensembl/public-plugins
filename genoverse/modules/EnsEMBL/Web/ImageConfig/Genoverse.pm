...
sub init_genoverse {
  ...
  $self->modify_configs([ 'chr_band_core'                                ], { genoverse => { autoHeight => 'force',               cache          => 'chr',       allData => JSON::true      } });
  ...
  $_->set('genoverse', { bump => JSON::true }) for grep $_->id =~ /^qtl_/, $node ? $node->nodes : ();
}
