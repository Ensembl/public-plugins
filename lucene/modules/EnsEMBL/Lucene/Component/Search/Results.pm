...
sub re_search {
    ...
    if ( !$do_search && ( $species ne 'all' ) ) {
        $species =~ s/_/ /g;
        $html = qq(<h2>Your search of $display_species with '$q' returned no results</h2>);
        my $url = '/' . $hub->species . '/Search/Results?species=all;idx=' . $hub->param('idx') . ';q=' . $q;
        $html .= sprintf qq(<h3>Would you like to search <a href="%s">all species</a> with this term ?</h3>), $url;
        return $html;
    }
