...
Ensembl.GA4 = {
  ...
  eventConfigs: [],
  /*
   * List of configs registered by default (added later in ga4_configs.js)
   */
  ...
  sendEvent: function (config, extra, e) {
    ...
    Ensembl.GA4.EventConfig.destroy(myConfig);
  },
  ...
  registerConfigs: function (eventConfigs) {
    ...
    eventConfigs = $.makeArray($.map(eventConfigs, function (config) {
      return config.deleted || config.url && !window.location.href.match(config.url) ? null : new Ensembl.GA4.EventConfig(config);
    }));
 ...
 */
Ensembl.GA4.EventConfig = function (config) {
  $.extend(this, config);
};

Ensembl.GA4.EventConfig.prototype.getURL = function (url) {
  return Ensembl.GA4.filterURL(url || this.currentTarget || this.currentOptions.url);
};

Ensembl.GA4.EventConfig.prototype.getText = function (el) {
  return $(el || this.currentTarget).text().trim();
};

Ensembl.GA4.EventConfig.prototype.instantiate = function (extra, defaults, e) {
  ...
  $.each(config.data, function (key, val) { // resolve data first to allow it's use when resolving other keys
    Ensembl.GA4.EventConfig.resolve(config, e, config.data, key);
  });
  ...
  $.each(['category', 'action', 'label', 'value', 'nonInteraction'], function (i, key) { // order is kept like this intentionally to allow use of category, action in other keys
    Ensembl.GA4.EventConfig.resolve(config, e, config, key);
  });

  return config;
};

Ensembl.GA4.EventConfig.resolve = function (config, e, obj, key) {
  if (typeof obj[key] === 'function') {
    obj[key] = obj[key].call(config, e);
  }
};

Ensembl.GA4.EventConfig.destroy = function (obj) {
  if (obj.data) {
