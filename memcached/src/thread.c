 ...
 *
 *  $Id: thread.c,v 1.1 2008/10/02 13:30:43 eb4 Exp $
 */
#include "memcached.h"
#include <stdio.h>
#include <errno.h>
 ...
 */
void mt_item_flush_by_tags(const char *tags[], const size_t *ntags[], const uint8_t n) {
    pthread_mutex_lock(&cache_lock);
    do_item_flush_by_tags(tags, ntags, n);
    pthread_mutex_unlock(&cache_lock);
}
