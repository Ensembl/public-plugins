Ensembl.Panel.BlastForm = Ensembl.Panel.Content.extend({
  ...
  init: function () {
    ...
    this.elLk.blastForm       = this.el.find('form.blast_form').on('submit', function(e) {
      e.preventDefault();
      $.ajax({
        'url'       : this.action,
        'method'    : 'post',
    ...
    this.maxSequenceLength    = this.elLk.blastForm.find('input[name=max_sequence_length]').remove().val();
    this.maxNumSequences      = this.elLk.blastForm.find('input[name=max_number_sequences]').remove().val();
    this.dnaThresholdPercent  = this.elLk.blastForm.find('input[name=dna_threshold_percent]').remove().val();

    try {
      // parse the combination JSON from the HTML - if this doesn't work, there is nothing we can do!
      this.combinations       = $.parseJSON(this.elLk.blastForm.find('input[name=valid_combinations]').remove().val());

      // get the fields order from the order they appear on the form
      this.elLk.blastForm.find($.map(this.combinations[0], function(val, key) {
        return '[name=' + key + ']';
    ...
    if (!this.combinations || !this.maxSequenceLength || !this.dnaThresholdPercent || !this.maxNumSequences) {
      this.showError('JavaScript error occurred while initiating the Blast form.');
      return;
    }

    this.elLk.sequences       = this.elLk.blastForm.find('div._sequence').first().removeClass('_sequence');
    this.elLk.sequenceFields  = this.elLk.blastForm.find('div._sequence');
    this.elLk.speciesDropdown = this.elLk.blastForm.find('div._species_dropdown');
    this.elLk.adjustableDivs  = this.elLk.blastForm.find('div._adjustable_height').css('minHeight', function() { return $(this).height(); });
    this.elLk.configFields    = this.elLk.blastForm.find('div._config_field');
    this.elLk.addSeqLink      = this.elLk.sequences.find('._add_sequence').on('click', function(e) {
      e.preventDefault();
      panel.toggleSequenceFields(true);
    });
    this.elLk.queryType       = this.elLk.blastForm.find('input[name=query_type]').on('change', function() {
      panel.setQueryTypeValue(this.value);
    });

    // species tags
    this.elLk.speciesTags     = this.elLk.blastForm.find('._species_tags').find('div').each(function() {
      panel.speciesTags[$(this).find('input').val()] = new Ensembl.Panel.BlastForm.SpeciesTag($(this), panel);
    }).end();

    // 'Add species' link
    this.elLk.addSpeciesLink  = this.elLk.blastForm.find('._add_species').find('a').on('click', function(e) {
      e.preventDefault();
      panel.toggleSpeciesFields(true);
    }).end();

    // 'Done' link - done adding species
    this.elLk.doneSpeciesLink = this.elLk.blastForm.find('._add_species_done').find('a').on('click', function(e) {
      e.preventDefault();
    ...
    this.elLk.blastForm.find('._validate_onchange').on('change', function() {
      panel.updateSelections($(this), true);
    }).filter('select').each(function() {
      panel.dropdowns[this.name] = new Ensembl.Panel.BlastForm.Dropdown($(this), panel);
    });

    // provide event handlers to the textare where sequence text is typed
    this.elLk.blastForm.find('textarea[name=query_sequence]').on({
      'focus': function(e) {
...
*/
    ...
    this.elLk.blastForm.find('input[name=query_file]').on('change', function() {
    ...
    this.elLk.blastForm.find('input[name=retrieve_accession]').on('change', function() {
        // TODO - accession ID
    });

    // initialise the FieldTag instances
    this.elLk.blastForm.find('select, input').each(function() {
      var tagDiv = panel.elLk.blastForm.find('._tag_' + this.name);
      if (tagDiv.length) {
  ...
  refreshSpecies: function() {
    ...
    for (speciesName in selectedSpecies) {
      if (!(speciesName in this.speciesTags)) {
        this.speciesTags[speciesName] = new Ensembl.Panel.BlastForm.SpeciesTag(false, this, existingTag, {'value': speciesName, 'caption': selectedSpecies[speciesName]});
    ...
    if (!skipFiltering) {
      ...
      for (fieldName in availableValues) {
        if (fieldName == 'species') {
          ...
          field2 = this.elLk.blastForm.find('select[name=' + fieldName + '] option, input[name=' + fieldName + ']').prop('disabled', function() {
  ...
  showBusy: function(flag) {
    if (!this.elLk.busyDiv) {
      var offset = this.elLk.blastForm.offset();
      this.elLk.busyDiv = $('<div class="form-overlay">').css({'left': offset.left, 'top': offset.top, 'height': this.elLk.blastForm.height(), 'width': this.elLk.blastForm.width()}).appendTo(document.body);
...
Ensembl.Panel.BlastForm.SubElement = Base.extend({
/*
...
Ensembl.Panel.BlastForm.Sequence = Ensembl.Panel.BlastForm.SubElement.extend({
/*
...
Ensembl.Panel.BlastForm.FieldTag = Base.extend({ // TODO extend from SubElement
/*
...
Ensembl.Panel.BlastForm.Dropdown = Base.extend({ // TODO extend from SubElement
/*
...
Ensembl.Panel.BlastForm.SpeciesTag = Base.extend({ // TODO extend from SubElement
