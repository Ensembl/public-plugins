...
Ensembl.Panel.ZMenu = Ensembl.Panel.extend({
          ...
          if (timeout === this.timeout) {
            ...
            if (json.entries.length) {
              ...
              if (json.highlight.length){
                ...
                this.relatedEl = $('tr.' + json.highlight,  this.base);
                this.relatedEl.addClass('highlight');    
              }
  
              if(json.pagination.hasOwnProperty("position")){
  ...
  addPagination: function (pagination) {
    ...
    var count_string = 'Showing ' + index + ' of ' + total + ' features';  

    $('span', this.elLk.nav).addClass('paginate_button').removeClass('paginate_active paginate_button_disabled').css('display', '').unbind('click');

    this.p1_button        =  $('span.p1', this.el); 
    this.p2_button        =  $('span.p2', this.el);
    this.p3_button        =  $('span.p3', this.el);

    if (p2 == total){ 
      this.p3_button.html(p2).removeClass('paginate_button').addClass('paginate_active');
      if ( p1 >= 1){
        this.p2_button.html(p1);  
      } 
      if (p0 >> 0){
        this.p1_button.html(p0);
      } else { 
        this.p1_button.css('display', 'none');
      }
    } else if (p2 == 1) {
      this.p1_button.html(p2).removeClass('paginate_button').addClass('paginate_active');
      if (p3 <= total){
        this.p2_button.html(p3);
      }
      if (p4 <= total){
        this.p3_button.html(p4);
      } else {
        this.p3_button.css('display', 'none');
      } 
    } else {
      this.p1_button.html(p1);
      this.p2_button.html(p2).removeClass('paginate_button').addClass('paginate_active');
      this.p3_button.html(p3);
    }
      
    //disable buttons that aren't valid
    if (p2 == 1){
      $('span.first', this.elLk.nav).addClass('paginate_button_disabled');
      $('span.previous', this.elLk.nav).addClass('paginate_button_disabled');
    } 

    if (p2 == total){
      $('span.next', this.elLk.nav).addClass('paginate_button_disabled');
      $('span.last', this.elLk.nav).addClass('paginate_button_disabled');    
    }


    this.elLk.count.addClass('zmenu_bottom');
    this.elLk.nav.css('display', 'block');
    this.elLk.count.html(count_string);

    $('span.paginate_button', this.elLk.nav).on('click', function () { 
      var url = pagination.url_template;
      var new_index;
      ...
      url = url.replace(/idx=\d*/, 'idx=' + new_index);
      panel.populateAjax(url);
    });

    $('span.paginate_button_disabled', this.elLk.nav).unbind('click');
