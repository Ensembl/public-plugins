      ...
      if($highlight_set->{$chr}) {
        ...
        if($highlight_set->{'merge'} && $highlight_set->{'merge'} eq 'no') {
          ...
          foreach (@temp_highlights) {
            ...
            if ($_->{'href'} =~/bin/){
              my $href = $_->{'href'};
              my $b = sprintf '%d', $bin_id; 
              my $bin_start = sprintf '%d', ($b * $bin_length);
              $bin_start =  $bin_start - $v_offset;
              my $bin_end = sprintf '%d', ($bin_start + $bin_length);
              my $bin_coords = $chr .':'.$bin_start .'-' . $bin_end;
              $href =~s/bin=\d*/bin=$bin_coords/;
              $_->{'href'} = $href;   
            }
            $is_aggregated = 0;
          }
