...
package EnsEMBL::Web::Object::DataSlicer;
...
sub species_list {
  ...
  if (!$self->{'_species_list'}) {
    ...
    for ($self->valid_species) {
      next if $_ ne "Homo_sapiens"; #restricting species list to human only, remove this if you want full list of species
...
### Uses Controller::Download, via url /Download/DataSlicer/
### 2 files to download (bam (.bam) and index (.bai) file)
  ...
  my $filename    = $hub->param('bai') ? $job->dispatcher_data->{'output_file'}.".bai" : $job->dispatcher_data->{'output_file'};
