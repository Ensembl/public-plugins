...
sub content {
  ...
  my $genomic_hit   = $self->hit;
  my $species       = $genomic_hit->{'species'};
  ...
  foreach (qw(exon_ori match_display snp_display line_numbering codons_display title_display exon_display)) {
    $config->{$_} = $hub->param($_) unless $hub->param($_) && ($hub->param($_) eq 'off' || $hub->param($_) eq 'no');
  }

  $config->{'end_number'}   = $config->{'number'} = 1 if $config->{'line_numbering'};
  $config->{'exon_display'} = 'selected' if $config->{'exon_ori'};
  $config->{'query_seq'}    = 'on';
  $config->{'slices'}       = $self->get_alignment_slices($job_data, $genomic_hit, $species, $config);
...
sub get_alignment_slices {
  my ($self, $job_data, $hit, $species, $config) = @_;
  my $object        = $self->object;
  my $blast_method  = $self->blast_method;
  my $homology_string;
  ...
  if ($self->is_protein) {
    ...
    @slices = (
      { name => 'Query', slice => $query_slice, no_markup => 1 },    
      { name => '', slice => $query_slice, seq => $homology_string, no_markup => 1, no_numbers => 1},
      { name => 'Subject', slice => $slice, seq => $hit_slice->seq, msc => $mapped_slice_container },
    ); 
...
sub get_mapped_slice {
  ...
  my $aln = $hit->{'db_type'} =~/cdna/i ?  $object->map_btop_to_genomic_coords($hit, $self->hit_id) : $hit->{'aln'};
